...
import type { SKAL VenturesEvent } from "#/types/agent-server/core";
...
function makeEvent(id: string, timestamp: string): SKAL VenturesEvent {
  return { id, timestamp } as unknown as SKAL VenturesEvent;
}

function makePage(
  items: SKAL VenturesEvent[],
  nextPageId: string | null = null,
): EventSearchPage<SKAL VenturesEvent> {
...
describe("useLoadOlderEvents", () => {
  ...
  it("seeds inline model-switch messages for switches in a paginated older page", async () => {
    ...
    const userMsg = {
      ...
      extended_content: [],
    } as unknown as SKAL VenturesEvent;
    const switchObs = {
      id: "evt-switch-old",
  ...
  it("keeps paginating while the server keeps returning full pages", async () => {
    ...
    const fullPage: SKAL VenturesEvent[] = Array.from(
  ...
  it("coalesces repeated loadOlder calls while a page request is already in flight", async () => {
    ...
    let resolvePage!: (page: EventSearchPage<SKAL VenturesEvent>) => void;
    const pendingPage = new Promise<EventSearchPage<SKAL VenturesEvent>>(
      (resolve) => {
        resolvePage = resolve;
      },
    );
  ...
  it("throws a descriptive error when searchEvents returns malformed items", async () => {
    ...
    vi.spyOn(EventService, "searchEvents").mockResolvedValue({
      items: { bad: true },
      next_page_id: null,
    } as unknown as EventSearchPage<SKAL VenturesEvent>);
  ...
  it("stops paginating silently when the oldest loaded event is missing a timestamp", async () => {
    act(() => {
      useEventStore
        .getState()
        .addEvent({ id: "evt-missing-ts" } as SKAL VenturesEvent);
    });
