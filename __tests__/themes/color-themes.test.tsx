...
describe("color themes", () => {
  it("includes SKAL Ventures-Neo as a neutral-based theme with white button tokens", () => {
    const neo = COLOR_THEMES["openhands-neo"];

    expect(neo.label).toBe("SKAL Ventures-Neo");
  ...
  it("exposes Neo in the settings theme picker", () => {
    ...
    expect(
      AVAILABLE_COLOR_THEMES.find((theme) => theme.key === "openhands-neo")
        ?.label,
    ).toBe("SKAL Ventures-Neo");
  });

  it("injects white primary tokens when applying SKAL Ventures-Neo", () => {
