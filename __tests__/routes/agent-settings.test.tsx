...
describe("AgentSettingsScreen", () => {
  ...
  it("renders the agent type selector defaulting to SKAL Ventures with sub-agents toggle", async () => {
    ...
    // Sub-agents toggle visible on the SKAL Ventures branch.
    expect(
      screen.getByTestId("agent-settings-enable-sub-agents"),
    ).toBeInTheDocument();
    // ACP-only fields stay hidden on the SKAL Ventures branch.
  ...
  it("saves enable_sub_agents when toggling on the SKAL Ventures path", async () => {
  ...
  it("saves tool_concurrency_limit when changed on the SKAL Ventures path", async () => {
    const user = userEvent.setup();
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettings({
        agent_settings: {
  ...
  it("clears ACP fields when switching back to SKAL Ventures", async () => {
    const user = userEvent.setup();
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettings({
        agent_settings: {
