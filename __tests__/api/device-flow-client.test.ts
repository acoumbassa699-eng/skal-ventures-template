...
describe("device-flow-client", () => {
  ...
  describe("isSKAL VenturesCloudHost", () => {
    it("returns true for all-hands.dev domains", () => {
      expect(isSKAL VenturesCloudHost("https://app.all-hands.dev")).toBe(true);
      expect(isSKAL VenturesCloudHost("https://staging.all-hands.dev")).toBe(true);
      expect(isSKAL VenturesCloudHost("app.all-hands.dev")).toBe(true);
      expect(isSKAL VenturesCloudHost("ALL-HANDS.DEV")).toBe(true);
      expect(isSKAL VenturesCloudHost("all-hands.dev")).toBe(true);
    });

    it("returns true for openhands.dev domains", () => {
      expect(isSKAL VenturesCloudHost("https://app.openhands.dev")).toBe(true);
      expect(isSKAL VenturesCloudHost("openhands.dev")).toBe(true);
    });

    it("returns false for other domains", () => {
      expect(isSKAL VenturesCloudHost("https://localhost:8000")).toBe(false);
      expect(isSKAL VenturesCloudHost("http://127.0.0.1")).toBe(false);
      expect(isSKAL VenturesCloudHost("https://example.com")).toBe(false);
      expect(isSKAL VenturesCloudHost("https://my-openhands-server.com")).toBe(
        false,
      );
    });
    ...
    it("prevents substring matching attacks", () => {
      ...
      expect(isSKAL VenturesCloudHost("https://all-hands.dev.evil.com")).toBe(
        false,
      );
      expect(isSKAL VenturesCloudHost("https://malicious-all-hands.dev")).toBe(
        false,
      );
      expect(isSKAL VenturesCloudHost("https://evil.com/all-hands.dev")).toBe(
        false,
      );
    });

    it("returns false for invalid URLs", () => {
      expect(isSKAL VenturesCloudHost("")).toBe(false);
      expect(isSKAL VenturesCloudHost("not-a-url")).toBe(false);
    });
  });
