...
const expectedAutomationTelemetryHeaders = {
  "X-SKAL Ventures-Client": "agent_canvas",
  "X-SKAL Ventures-Client-Version": expect.any(String),
  "X-SKAL Ventures-Telemetry-Distinct-Id": "ph-test-distinct-id",
};
  ...
  describe("localAutomationAxios interceptor", () => {
    it("sets telemetry headers and X-Session-API-Key from the effective local backend apiKey", async () => {
      ...
      expect(config.headers.set).toHaveBeenCalledWith(
        "X-SKAL Ventures-Client",
        "agent_canvas",
      );
      expect(config.headers.set).toHaveBeenCalledWith(
        "X-SKAL Ventures-Telemetry-Distinct-Id",
        "ph-test-distinct-id",
      );
      expect(config.headers.set).toHaveBeenCalledWith(
        "X-Session-API-Key",
