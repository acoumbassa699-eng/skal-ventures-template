...
describe("buildStartConversationRequest", () => {
  ...
  it("uses nested settings as the source of truth and lets the SDK create the agent", () => {
    ...
    expect(payload.agent_settings.llm).toMatchObject({
      ...
      api_key: "nested-key",
      base_url: "https://nested.example.com",
      // Streaming is enabled so the SKAL Ventures agent emits StreamingDeltaEvents.
      stream: true,
    });
  ...
  it("derives confirmation and security settings the same way as SKAL Ventures", () => {
    const payload = buildStartConversationRequest({
      settings: {
        ...DEFAULT_SETTINGS,
        agent_settings: {
  ...
  it("does not mirror conversation secrets onto agent_context for non-ACP conversations", () => {
    // The SKAL Ventures ``Agent`` reads secrets from ``secret_registry``
  ...
  describe("canvas_ui client tool injection", () => {
    ...
    it("sends the client tool for an SKAL Ventures profile", () => {
      const payload = buildStartConversationRequest({
        settings: DEFAULT_SETTINGS,
...
describe("toAppConversation", () => {
  ...
  it("ignores tags.acpserver on SKAL Ventures conversations to prevent stray-tag bleed", () => {
    ...
    // turning the sidebar of an SKAL Ventures conversation into "Claude
...
describe("buildStartConversationRequest — ACP discriminator", () => {
  ...
  it("does not include ACP-only fields in SKAL Ventures agent settings", () => {
    const payload = buildStartConversationRequest({
      settings: {
        ...DEFAULT_SETTINGS,
        agent_settings: {
  ...
  it("ACP → SKAL Ventures → ACP round trip leaves no field leakage", () => {
