...
describe("mergeAgentProfileSaveInput", () => {
  it("preserves unmodeled SKAL Ventures fields under the edited ones", () => {
    ...
    const merged = mergeAgentProfileSaveInput(storedSKAL Ventures, edited);
  ...
  it("strips server-managed identity from the merge", () => {
    const merged = mergeAgentProfileSaveInput(storedSKAL Ventures, {
      agent_kind: "openhands",
      enable_sub_agents: true,
      llm_profile_ref: "new-llm",
    });
  ...
  it("sends a clean variant payload on an agent_kind switch", () => {
    ...
    expect(mergeAgentProfileSaveInput(storedSKAL Ventures, edited)).toEqual(edited);
  });
