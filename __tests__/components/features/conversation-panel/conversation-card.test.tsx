...
describe("ConversationCard", () => {
  ...
  describe("Agent chip", () => {
    ...
    // toggle (``showLlmProfiles``) — one control for both ACP and SKAL Ventures
    ...
    it("hides the chip when showLlmProfiles is omitted for SKAL Ventures", () => {
      renderWithProviders(
        <ConversationCard
          title="Conversation 1"
          selectedRepository={null}
    ...
    it("renders the SKAL Ventures logo + model name for native conversations", () => {
      // With the chip toggle on, SKAL Ventures native conversations show the
      // SKAL Ventures logo + the raw ``agent.llm.model`` string. A stray
      // ``acp_server`` value on an SKAL Ventures card must not flip the icon to
    ...
    it("hides the chip for SKAL Ventures conversations with no model", () => {
