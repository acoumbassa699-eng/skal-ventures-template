...
describe("BackendSelector", () => {
  ...
  it("shows every organization as a separate row for cookie-auth cloud backends", async () => {
    ...
    await waitFor(() => {
      const wrapper = screen.getByTestId("backend-selector");
      const input = wrapper.querySelector("input") as HTMLInputElement;
      expect(input.value).toBe("SKAL Ventures Cloud – Acme Inc");
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText("SKAL Ventures Cloud – Personal"),
      ).toBeInTheDocument();
    });
    expect(screen.getByText("SKAL Ventures Cloud – Acme Inc")).toBeInTheDocument();
