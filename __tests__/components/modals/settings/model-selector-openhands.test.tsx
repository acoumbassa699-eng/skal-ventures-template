...
describe("ModelSelector — SKAL Ventures provider display", () => {
  ...
  it("shows SKAL Ventures immediately for current openhands/<m> settings", async () => {
    renderWithQuery(<ModelSelector currentModel="openhands/claude-opus-4-7" />);

    await waitFor(() => {
      expect(screen.getByLabelText("LLM$PROVIDER")).toHaveValue("SKAL Ventures");
    });
