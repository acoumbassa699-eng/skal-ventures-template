...
async function fillForm(user: ReturnType<typeof userEvent.setup>) {
  await user.type(
    ...
    "SKAL Ventures/agent-server-gui",
  );
...
describe("SetupDialog", () => {
  ...
  it("creates from the derived payload and opens what was created", async () => {
    ...
    expect(mocks.runAction.mock.calls[0][2]).toEqual({
      name: "Widget monitor - SKAL Ventures/agent-server-gui",
      prompt: "Report on Widgets in SKAL Ventures/agent-server-gui.",
      repos: [{ url: "SKAL Ventures/agent-server-gui", provider: "github" }],
