...
import {
  ...
  deleteConversation,
  resetToSKAL VenturesAgentViaUI,
  resetMockLLM,
...
test.describe("mock-LLM ACP agent conversation", () => {
  ...
  test.afterAll(async ({ request, browser }) => {
    ...
    // Reset agent-server back to SKAL Ventures via the Settings → Agent UI
    ...
    try {
      ...
      await resetToSKAL VenturesAgentViaUI(page);
  ...
  test("step 1: configure ACP agent via Settings → Agent UI", async ({
    ...
    // ── Switch agent type from SKAL Ventures → ACP ──
