...
export async function ensureMockLLMProfile(
  ...
  try {
    ...
    // default agent is already SKAL Ventures, so the LLM settings route stays usable.
  }
...
export async function ensureMockLLMAgentProfile(
  ...
  const saveResp = await retryOnTransient(
    ...
    {
      headers,
      // disabled_skills omitted: an SKAL Ventures profile launches with all
...
/**
 * Reset agent type back to SKAL Ventures through the Settings → Agent profiles UI.
 * Used in afterAll cleanup to restore the default agent for subsequent tests.
 */
export async function resetToSKAL VenturesAgentViaUI(page: Page) {
  await openAgentProfileEditor(page, "default");

  await selectDropdownOption(page, /Agent/, /SKAL Ventures/);
...
/**
 * @deprecated Use `resetToSKAL VenturesAgentViaUI(page)` to exercise the UI path.
 * Kept only for callers that cannot open a page (should not exist in new tests).
 */
export async function resetToSKAL VenturesAgent(request: APIRequestContext) {
  ...
  if (!resp.ok()) {
    console.warn(`[cleanup] Reset to SKAL Ventures failed: ${resp.status()}`);
  }
}
