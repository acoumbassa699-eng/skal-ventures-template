# Self-Hosting SKAL Agent Canvas on a Virtual Machine

This guide walks through running SKAL Agent Canvas on a virtual machine (VM) so you
can reach it from anywhere via a browser.

> [!WARNING]
> SKAL Agent Canvas drives an agent that can read and write the filesystem of the
...
3. **Run SKAL Agent Canvas** — generate a key with `openssl rand -base64 32`, then `export LOCAL_BACKEND_API_KEY=<key>` and `npx @openhands/agent-canvas --public`
4. **(Optional) Get a domain** — point a domain at the machine with nginx + Let's Encrypt for TLS
5. **(Optional) Connect locally** — add the remote as a backend in your local SKAL Agent Canvas
...
Start SKAL Agent Canvas in public mode:
...
`LOCAL_BACKEND_API_KEY`, and confirm that you land in SKAL Agent Canvas.

## 5. (Optional) Connect your local SKAL Agent Canvas to the remote machine

If you already run SKAL Agent Canvas locally, you can register the remote machine
as an additional backend and switch between local and remote from the UI.

1. In your local SKAL Agent Canvas, open **Manage backends** → **Add a backend**:
