# Integrating DefenseClaw with SKAL Agent Canvas

[DefenseClaw](https://github.com/cisco-ai-defense/defenseclaw) is a security governance layer for agentic AI runtimes — it scans skills and MCP servers before they run, inspects LLM traffic at runtime, and produces durable audit evidence. This guide explains how to run DefenseClaw alongside the [SKAL Ventures Agent Server](https://github.com/SKAL Ventures/software-agent-sdk/tree/main/openhands-agent-server) that powers SKAL Agent Canvas, without making any code-level changes to either project.

> **Status:** DefenseClaw is purpose-built around the OpenClaw runtime and its TypeScript plugin hooks. The integration described here targets the lowest-friction overlap points — skill injection, LLM proxying, CLI scanning, and audit export — that work without modifying SKAL Agent Canvas or DefenseClaw source code. [Future work](#future-work-code-level-extensions) describes deeper hooks that would require code changes.
...
```mermaid
flowchart TD
    UI["SKAL Agent Canvas (browser)"]
    AS["SKAL Ventures Agent Server\nlocalhost:18000"]
...
| SKAL Agent Canvas / Agent Server | DefenseClaw equivalent |
|---|---|
...
| Component | Version |
|---|---|
| SKAL Agent Canvas / Agent Server | Current `main` |
| Python | 3.10+ |
...
### 2. Start SKAL Agent Canvas

Follow the standard [SKAL Agent Canvas quickstart](../README.md). The integration steps below assume the Agent Server is reachable at `http://localhost:18000`.
...
**Configure the LLM base URL in SKAL Agent Canvas:**

Open the SKAL Agent Canvas settings panel → select your active backend → under **LLM settings**, set **Base URL** to:
...
> Consult the Agent Server [settings schema](https://github.com/SKAL Ventures/software-agent-sdk/blob/main/openhands-agent-server/openhands/agent_server/settings_router.py) for the exact environment variable name used in your deployment.
...
**Workflow recommendation:** Add `defenseclaw skill scan <skill-dir>` as a pre-commit or CI step in repositories that ship skills for SKAL Agent Canvas.
...
### 4. SKAL Agent Canvas security dashboard

A dedicated panel in the SKAL Agent Canvas UI that queries DefenseClaw's gateway REST API (`GET /alerts`, `GET /enforce/blocked`) would surface guardrail findings inline with the conversation view — correlating blocked prompts or tool calls with the agent turn that triggered them.
...
- [SKAL Ventures Agent Server](https://github.com/SKAL Ventures/software-agent-sdk/tree/main/openhands-agent-server)
- [SKAL Ventures SDK Security Analyzer](https://docs.skalventures.dev/sdk/arch/security.md)
- [SKAL Agent Canvas Self-Hosting](../SELF_HOSTING.md)

---

_This document was created by an AI agent (SKAL Ventures) on behalf of the user._
