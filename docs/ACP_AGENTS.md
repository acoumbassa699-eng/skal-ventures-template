...
SKAL Agent Canvas can drive your conversations with the built-in **SKAL Ventures** agent or
...
SKAL Agent Canvas calling an LLM directly, the Agent Server spawns the agent's own CLI
as a subprocess and relays each turn to it. The external agent manages its own
LLM, tools, and execution; SKAL Agent Canvas sends messages and renders what comes
back.

```mermaid
flowchart LR
    canvas["SKAL Agent Canvas<br/>(this UI)"]
    server["Agent Server"]
...
The Agent Server owns the subprocess and the credentials; SKAL Agent Canvas only
...
1. **Choose agent** — pick Claude Code, Codex, or Gemini CLI instead of
   SKAL Ventures. The choice is saved immediately to your backend's settings.
2. **Check backend** — confirms SKAL Agent Canvas can reach the Agent Server.
...
- **Agent** — switch between **SKAL Ventures** and **ACP**.
