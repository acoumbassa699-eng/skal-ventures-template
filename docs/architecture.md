# SKAL Agent Canvas architecture

SKAL Agent Canvas is a React and TypeScript frontend for running and monitoring SKAL Ventures agents across local, remote, and hosted environments. It is adapted from the SKAL Ventures frontend to talk directly to the SKAL Ventures Agent Server and related automation services.

## System boundaries

SKAL Agent Canvas is responsible for:
...
- Translating UI actions into SKAL Ventures Agent Server API calls.
- Packaging the UI both as a standalone app and as library entrypoints for host applications.

SKAL Agent Canvas is not responsible for:
...
The primary backend is the [SKAL Ventures Agent Server](https://github.com/SKAL Ventures/software-agent-sdk/tree/main/openhands-agent-server/openhands/agent_server). SKAL Agent Canvas can connect to one or more Agent Server instances and switch between them from the UI.
...
- SKAL Ventures Cloud APIs for hosted sandbox and organization workflows.

SKAL Agent Canvas stack launchers expose runtime service information through the backend `/server_info.runtime_services` field. The frontend forwards that backend-provided information into new conversations as an agent context suffix so agents can use the correct URLs instead of guessing ports.
...
SKAL Agent Canvas supports several modes:
...
| `npm run build:lib` | Builds library entrypoints for embedding SKAL Agent Canvas components. |
