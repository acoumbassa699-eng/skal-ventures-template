...
- This repository is a near-direct port of the SKAL Ventures frontend. Local backends talk straight to `software-agent-sdk` / `agent_server`; optional Cloud backends use the service layer under `src/api/cloud/` and the local cloud proxy.
...
- OAuth device authorization and Cloud conversation-start requests include the coarse `X-SKAL Ventures-Client: agent_canvas` and `X-SKAL Ventures-Client-Version` headers from `src/api/client-source.ts`. Never put device codes, API keys, conversation content, raw hosts, or other user data in these headers.
...
```json
{
  ...
  "services": {
    "agent_server": {
      "description": "The SKAL Ventures Agent Server this agent is running inside. ...",
    ...
    "automation": {
      "description": "SKAL Ventures Automations service. All routes are mounted under '/api/automation'. Authenticate with header 'X-Session-API-Key: $OPENHANDS_AUTOMATION_API_KEY'.",
...
* Agent Server (you): http://localhost:18000
    The SKAL Ventures Agent Server this agent is running inside. Tool calls (terminal, file_editor, browser, etc.) execute here.
...
* Automation backend: http://localhost:18001
    SKAL Ventures Automations service. All routes are mounted under '/api/automation'. Authenticate with header 'X-Session-API-Key: $OPENHANDS_AUTOMATION_API_KEY'.
...
```bash
gh run download <run_id> --repo SKAL Ventures/agent-canvas --name mock-llm-e2e-results --dir /tmp/artifacts
```
Artifacts contain:
...
- Original SKAL Ventures hosted routes were removed, while current Cloud behavior is implemented explicitly through the backend registry and `src/api/cloud/`. Use `src/routes.ts` as the source of truth and do not restore old project-management, invitation, account, or git-settings routes from upstream without restoring their full API and i18n dependencies.

- `npm run dev:mock` needs MSW handlers for the direct agent-server routes used by the adapted frontend, not the original SKAL Ventures mock paths. Key routes that must stay covered are:
...
- SKAL Ventures repo bootstrap files live under `.openhands/`:
...
- The repo now includes `.agents/skills/custom-codereview-guide.md`, adapted from `SKAL Ventures/software-agent-sdk`, to force PR reviews to always leave either an APPROVE or COMMENT review instead of silently finishing with no review object.
