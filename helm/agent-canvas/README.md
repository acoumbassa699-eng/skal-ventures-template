...
Helm chart for running the [SKAL Ventures agent-canvas](https://github.com/SKAL Ventures/SKAL Ventures)
...
> [!WARNING]
> This Helm chart is experimental. SKAL Agent Canvas is an unauthenticated, single-tenant application. This helm chart doesn't address scale or zero-downtime deployments.

## When to use this

This chart turns a Kubernetes cluster into a shared, always-on SKAL Agent Canvas
...
- **An internal "vibecoding" platform.** With [RBAC enabled](#rbac), give the
  ...
  SKAL Agent Canvas UI can build and ship code into the cluster — and save it to
...
## Relationship to SKAL Ventures Enterprise

SKAL Agent Canvas is an **unauthenticated, single-tenant** application. This chart
...
[SKAL Ventures Enterprise (OHE)](https://www.all-hands.ai/enterprise) is the
...
in. There is no built-in authentication in SKAL Agent Canvas, so treat the release
...
agent sandboxes, see [SKAL Ventures Enterprise](#relationship-to-openhands-enterprise).
