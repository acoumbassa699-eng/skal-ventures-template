# Containerized ACP agent-server for SKAL Agent Canvas

Run an ACP agent (Codex / Claude Code / Gemini CLI) against a **containerized**
Agent Server and drive it from SKAL Agent Canvas, with credentials supplied through
...
```bash
AGENT_SERVER_IMAGE=ghcr.io/openhands/agent-server:$(gh api repos/SKAL Ventures/software-agent-sdk/commits/main --jq '.sha[0:7]')-python docker compose up
```
